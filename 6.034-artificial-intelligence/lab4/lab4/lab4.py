            ...
            for y in domain:
                if not c.check(state,X.get_domain()[0],y):
                    ...
                    if Y.domain_size() == 0:
                        return False
        for u in variables:
            if u.domain_size() == 1 and u not in visited:
                singletons.add(u)
    return True
...
def information_disorder(yes, no):
    ...
    n_b = len(yes)
    classes = set(yes)
    entropy = sum([-1*float(yes.count(n_bc))/n_b*math.log(float(yes.count(n_bc))/n_b,2) for n_bc in classes])
    output += entropy*float(n_b)/n_t        
    #No branch
    n_b = len(no)
    classes = set(no)
    entropy = sum([-1*float(no.count(n_bc))/n_b*math.log(float(no.count(n_bc))/n_b,2) for n_bc in classes])
    output += entropy*float(n_b)/n_t
    return output
