...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1,Product) and isinstance(expr2,Product):
        elements = list(expr1)+list(expr2)
        return Product(elements)
    elif isinstance(expr1,Product):
        for s in expr2:
            elements.append(Product(list(expr1)+[s]).simplify())
    elif isinstance(expr2,Product):
        for s in expr1:
            elements.append(Product(list(expr2)+[s]).simplify())
    else:
        for s1 in expr1:
            for s2 in expr2:
                elements.append(Product(list(s1)+list(s2)).simplify())
    return Sum(elements)